...
VIEWS.grn = {
  mod: 'store', render() {
    ...
    h += '<h2>Recent GRNs</h2><div class="tbl-wrap"><table><tr><th>GRN</th><th>Date</th><th>Inward</th><th>PO</th><th>Vendor</th><th>Invoice</th><th class="num">Accepted</th><th class="num">Rej + Short</th><th>By</th><th></th></tr>' +
      (gs.length ? gs.map(g => '<tr><td><b>' + esc(g.no) + '</b></td><td class="nowrap">' + fmtD(g.date) + '</td><td>' + esc(g.inward_no || '') + '</td><td>' + esc(g.po_no) + '</td><td>' + esc(g.vendor) + '</td><td>' + esc(g.invoice || '') + '</td><td class="num">' + qtyFmt(g.lines.reduce((s, l) => s + num(l.accepted), 0)) + '</td><td class="num late-txt">' + qtyFmt(g.lines.reduce((s, l) => s + num(l.rejected) + num(l.short || 0), 0)) + '</td><td>' + esc(g.by) + '</td><td class="right"><button class="btn sm ghost" data-act="print-grn" data-id="' + esc(g.id) + '">Print</button></td></tr>').join('') : '<tr><td colspan="10" class="empty">No GRNs yet</td></tr>') + '</table></div>';
    setMain(h);
  }
};
